# Technical reference: assurance for learned autonomous systems

**Status:** detailed companion to `whitepaper.md`, recovered from the pre-reader-first review edition rendered 25 September 2026. It preserves the full argument, source tags, worked-case detail, methods discussion, and compiler discussion for engineers who need to inspect them. The main paper is the recommended first read; this document is the expandable technical layer. It is a preserved detailed baseline, so its historical count/status prose must be reconciled to the current compiler report before any release.
...
- The learned-implementation gap and evidence bridge
- The road encounter, two-worlds argument and monitoring limit
- Evidence graph and eight contracts
- Existing methods, status landscape and coverage map
- Risk, scenario, calibration, runtime assurance and interpretability methods
- Full road assurance argument and road-to-air transfer
- Compiler, review agenda, current case status and source cards


## Detailed argument (preserved baseline)
...
The paper starts with a car approaching a partly hidden actor, then takes the same assurance structure into an airborne detect-and-avoid encounter. It uses AMLAS as a learned-component lifecycle baseline, compares current UK military guidance and proposed EASA civil material, and keeps their different scopes visible. <a href="#C-CHAL-001" class="claim-link" title="Open source and review card">[C-CHAL-001]</a> <a href="#C-CHAL-003" class="claim-link" title="Open source and review card">[C-CHAL-003]</a> <a href="#C-CHAL-004" class="claim-link" title="Open source and review card">[C-CHAL-004]</a> <a href="#C-CHAL-006" class="claim-link" title="Open source and review card">[C-CHAL-006]</a> The result is not a safety score or an approval recipe. It is a readable and machine-checkable argument about what evidence is still needed.

## How to read this prototype

The manuscript separates three kinds of statement:

- **Source-backed finding** has a stable claim tag such as <a href="#C-CHAL-006" class="claim-link" title="Open source and review card"><code>[C-CHAL-006]</code></a>; the interactive edition links each tag to a source card and the compiler review status.
...
With a trained model, the team still writes high-level requirements. It still designs the sensors, computer, interfaces, and surrounding software. But the detailed behavior of the model is learned from data. Its weights are the implementation. The FAA’s roadmap says the designer cannot derive lower-level requirements that directly describe that learned algorithm, or show that those requirements cover the higher-level ones in the usual way. <a href="#C-AUTH-010" class="claim-link" title="Open source and review card">[C-AUTH-010]</a>

<figure>
<img src="figures/learned_traceability_gap.svg" alt="A comparison of designed traceability, learned implementation, and the proposed evidence bridge." />
<figcaption aria-hidden="true">A comparison of designed traceability, learned implementation, and the proposed evidence bridge.</figcaption>
</figure>

*Figure 1. Simplified FAA framing at left and center; the green evidence bridge is this project’s proposal. Conventional assurance remains relevant for the software and hardware that run the model. The missing direct link concerns explaining the learned model’s internal behavior from lower-level requirements.* <a href="#C-AUTH-010" class="claim-link" title="Open source and review card">[C-AUTH-010]</a>
...
<figure>
<img src="figures/evidence_bridge_ladder.svg" alt="Six questions that form the project evidence bridge." />
<figcaption aria-hidden="true">Six questions that form the project evidence bridge.</figcaption>
</figure>
...
<figure>
<img src="figures/vision_training_contract.svg" alt="One versioned vision cue reused as an interface, with separate road and airborne training and safety evidence." />
<figcaption aria-hidden="true">One versioned vision cue reused as an interface, with separate road and airborne training and safety evidence.</figcaption>
</figure>
...
This is a project architecture, not a claim that a second sensor or a monitor automatically makes a system safe. If the monitor sees only the same missed object representation as the planner, it may share the failure. A separate box has added no information. The safety argument then needs an evidence-backed, sufficiently independent observation path, a bounded operational restriction, or a different allocation of decision authority. The conditional nature of runtime assurance is important: formal results about switching to a safe controller depend on monitor, timing, dynamics, and recovery premises. <a href="#C-EVID-005" class="claim-link" title="Open source and review card">[C-EVID-005]</a>
...
<figure>
<img src="figures/assurance_shells.svg" alt="Nested assurance scopes around a learned component, with an action envelope that is conditional on monitor observation and timing." />
<figcaption aria-hidden="true">Nested assurance scopes around a learned component, with an action envelope that is conditional on monitor observation and timing.</figcaption>
</figure>

*Figure 3. A learned component is only one part of a release-level assurance argument. The green action envelope is a project design pattern, not a claim that a generic deterministic wrapper makes a learned system safe. It must have sufficient observations, time, and control authority; a monitor can share the learned component’s blind spot.* <a href="#C-EVID-005" class="claim-link" title="Open source and review card">[C-EVID-005]</a>
...
|--------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Underfitting**                     | A model too limited for the chosen representation misses small or partly visible objects in development **and** independent tests.                     | Learning curves and errors by size, occlusion and background; reconsider capacity, features, data or the task. <a href="#C-TRAIN-004" class="claim-link" title="Open source and review card">[C-TRAIN-004]</a>                                                                                                          |
| **Overfitting or shortcut learning** | Development scores look strong, while a new route, sky, camera or weather condition fails; the network may use a simulator artifact or background cue. | A logged candidate-selection history; separate capture groups and shifted-condition tests; challenge the suspected cue. <a href="#C-TRAIN-002" class="claim-link" title="Open source and review card">[C-TRAIN-002]</a> <a href="#C-TRAIN-003" class="claim-link" title="Open source and review card">[C-TRAIN-003]</a> |
| **Label error or hidden ambiguity**  | Two labelers disagree about a partially occluded actor, or a hidden object is marked “clear” because no visible pixels reveal it.                      | Written label policy, adjudication record, disagreement sample and an explicit “unknown/occluded” outcome. <a href="#C-TRAIN-001" class="claim-link" title="Open source and review card">[C-TRAIN-001]</a>                                                                                                              |
| **Leakage**                          | Near-duplicate frames from one drive or flight appear on both sides of a split, or the supposedly final challenge set guides repeated tuning.          | Split by scene/capture event and provenance; locked verification set; record access and every tuning decision. <a href="#C-TRAIN-002" class="claim-link" title="Open source and review card">[C-TRAIN-002]</a> <a href="#C-TRAIN-003" class="claim-link" title="Open source and review card">[C-TRAIN-003]</a>          |
...
| **Safe cue, unsafe action**          | Detection succeeds but range/range rate, planner delay or maneuver clearance is wrong.                                                                 | Closed-loop encounter traces and the monitor’s independent observation and recovery margin. <a href="#C-EVID-005" class="claim-link" title="Open source and review card">[C-EVID-005]</a>                                                                                                                               |

The textbook distinction between underfitting and overfitting is a useful diagnostic: too little effective capacity can leave both training and test errors high; an overfit learning setup can show a large training-to-test gap. Real failures are not diagnosed from that graph alone. Data composition, optimization, labels and the test distribution also matter. A model that scores well on one independent set has demonstrated behavior on that set. It has **not** proven that it learned the intended physical concept rather than a convenient correlation, or that it will generalize to every future encounter. <a href="#C-TRAIN-004" class="claim-link" title="Open source and review card">[C-TRAIN-004]</a> <a href="#C-TRAIN-003" class="claim-link" title="Open source and review card">[C-TRAIN-003]</a>
...
<figure>
<img src="figures/assurance_coverage_map.png" alt="Source-scoped map of EASA proposed authority/risk boundaries, other public methods, and the project proposal." />
<figcaption aria-hidden="true">Source-scoped map of EASA proposed authority/risk boundaries, other public methods, and the project proposal.</figcaption>
</figure>

*Figure 5. A source-scoped coverage map. It shows documented scope, not acceptance, safety, maturity, or product approval. EASA NPA 2025-07(B) is proposed material. Its Level 3B is reserved, and its H1/fatality row is unacceptable across the proposed likelihood columns. AMLAS, MAA, FAA and NHTSA occupy different roles rather than competing “certification levels.”* \[C-RISK-006\] \[C-RISK-007\] <a href="#C-CHAL-001" class="claim-link" title="Open source and review card">[C-CHAL-001]</a> <a href="#C-CHAL-006" class="claim-link" title="Open source and review card">[C-CHAL-006]</a> <a href="#C-AUTH-010" class="claim-link" title="Open source and review card">[C-AUTH-010]</a> \[C-DIR-003\]
...
<figure>
<img src="figures/conditional_risk_graph.svg" alt="Conditional risk evidence graph, keeping exposure, performance, uncertainty and recovery evidence distinct before a bounded-authority decision." />
<figcaption aria-hidden="true">Conditional risk evidence graph, keeping exposure, performance, uncertainty and recovery evidence distinct before a bounded-authority decision.</figcaption>
</figure>
...
Combinatorial methods can make selected combinations of scenario factors visible and expose gaps in a test plan. That is valuable when the factors, bins, and interaction strength are justified. It does not prove coverage of a causal factor that the analyst did not include. <a href="#C-EVID-002" class="claim-link" title="Open source and review card">[C-EVID-002]</a>
...
Rare-event simulation can estimate a selected risk under a specified simulator, base distribution, and hazard threshold. It can make an otherwise impractical test problem tractable. Its result does not travel automatically to changed sensor rendering, a different tail distribution, or an environment whose dangerous cases are not represented by the generator. <a href="#C-EVID-003" class="claim-link" title="Open source and review card">[C-EVID-003]</a>
...
Formal runtime-assurance work can establish a safety implication for an advanced component when the monitor observes the relevant condition, detects it soon enough, the switch completes within a bound, dynamics stay within a model, and a fallback can reach a safe state. <a href="#C-EVID-005" class="claim-link" title="Open source and review card">[C-EVID-005]</a>
...
Saliency or feature-attribution visuals can help form hypotheses about why a model responded. Randomization checks show why a persuasive visualization must be tested for dependence on model and data structure before it is used as evidence. <a href="#C-EVID-006" class="claim-link" title="Open source and review card">[C-EVID-006]</a> Neuron-coverage research likewise provides a reason to investigate a metric, not a causal guarantee that higher activation coverage means lower hazardous behavior. <a href="#C-EVID-007" class="claim-link" title="Open source and review card">[C-EVID-007]</a>
...
**Claim R4 — hazardous-performance evidence.** Evaluation measures false negatives, false positives, delay, and calibration over scenario partitions that matter to the hazard, including shifted conditions. A rare-event result carries its simulator and distribution assumptions. This is `O-004`. <a href="#C-EVID-002" class="claim-link" title="Open source and review card">[C-EVID-002]</a> <a href="#C-EVID-003" class="claim-link" title="Open source and review card">[C-EVID-003]</a> <a href="#C-EVID-004" class="claim-link" title="Open source and review card">[C-EVID-004]</a>
...
**Claim R8 — change reopens evidence.** A change to training data, labels, weights, preprocessing, sensor hardware, planner, monitor, or the operating domain creates a new release and a declared impact assessment. This is `O-008`. <a href="#C-AUTH-002" class="claim-link" title="Open source and review card">[C-AUTH-002]</a>
...
The airborne case begins with five open assumptions: an airspace/traffic/weather/sensing envelope; a matching sensing and preprocessing configuration; sufficient remaining separation and maneuver margin after a late track; an evidence-backed, independent-enough traffic-and-clearance observation path for an active escape, or an operational restriction; and full change impact for learned weights, sensing, fusion, alerting, maneuver logic, and the operating domain. The associated evidence artifacts are still planned. The MAA path and EASA proposal give useful comparison points, but neither closes this case. Whether a particular DAA function falls within EASA’s stated high-consequence boundary depends on a system-level failure-condition allocation that this hypothetical case has not made. <a href="#C-CHAL-001" class="claim-link" title="Open source and review card">[C-CHAL-001]</a> <a href="#C-CHAL-003" class="claim-link" title="Open source and review card">[C-CHAL-003]</a> <a href="#C-CHAL-004" class="claim-link" title="Open source and review card">[C-CHAL-004]</a>

This is also where public claims about “certified DAA” require care. For this paper, equipment authorization, installation approval, an aircraft’s airworthiness basis, an operational permission, a military certification, and an underlying learned perception assurance argument are separate objects whose applicability depends on jurisdiction and product. In the narrower U.S. example, a TSO authorization is distinct from installation or operational use approval. <a href="#C-002" class="claim-link" title="Open source and review card">[C-002]</a> The paper will only make a product-specific statement after retrieving and reviewing a primary authorization whose scope can be stated precisely.
...
It is also wrong to convert that structure into a story that learned decision-making has already crossed every flight-critical boundary. The methods are conditioned by scope, assumptions, document status, and evidence. Within this project’s retrieved and inspected public sources as of 16 September 2026, we have not yet located a final, publicly inspectable civil lifecycle that closes the project’s highest-consequence hypothetical case. This is a dated retrieval result, bounded by the [retrieval log](../research/retrieval_status.md), not evidence about private programs, all jurisdictions, or future methods. <a href="#C-CHAL-003" class="claim-link" title="Open source and review card">[C-CHAL-003]</a> <a href="#C-CHAL-004" class="claim-link" title="Open source and review card">[C-CHAL-004]</a> <a href="#C-CHAL-006" class="claim-link" title="Open source and review card">[C-CHAL-006]</a>
...
- [AMLAS v1.1, University of York](https://www.york.ac.uk/media/assuring-autonomy/documents/AMLASv1.1.pdf) — learned-component methodology. <a href="#C-CHAL-006" class="claim-link" title="Open source and review card"><code>[C-CHAL-006]</code></a>
- [MAA/RN/2025/04, UK Military Aviation Authority](https://assets.publishing.service.gov.uk/media/68f0b97c1c9076042263ef2a/MAA_RN_2025_04.pdf) — case-specific military AI guidance. <a href="#C-CHAL-001" class="claim-link" title="Open source and review card"><code>[C-CHAL-001]</code></a>` `<a href="#C-CHAL-002" class="claim-link" title="Open source and review card"><code>[C-CHAL-002]</code></a>
- [EASA NPA 2025-07(B)](https://www.easa.europa.eu/en/downloads/142701/en) — proposed DS.AI material and scope. <a href="#C-CHAL-003" class="claim-link" title="Open source and review card"><code>[C-CHAL-003]</code></a>` `<a href="#C-CHAL-004" class="claim-link" title="Open source and review card"><code>[C-CHAL-004]</code></a>
- [FAA roadmap for AI safety assurance](https://www.faa.gov/aircraft/air_cert/step/roadmap_for_AI_safety_assurance) — static learned versus learning-in-operation distinction. <a href="#C-AUTH-002" class="claim-link" title="Open source and review card"><code>[C-AUTH-002]</code></a>
- [NIST AI RMF 1.0](https://doi.org/10.6028/NIST.AI.100-1) — voluntary risk-management framework. <a href="#C-EVID-001" class="claim-link" title="Open source and review card"><code>[C-EVID-001]</code></a>
- [NIST combinatorial-methods paper](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=957175) — selected-factor coverage methods. <a href="#C-EVID-002" class="claim-link" title="Open source and review card"><code>[C-EVID-002]</code></a>
- [Rare-event AV testing paper](https://arxiv.org/abs/1811.00145) — conditional rare-event simulation. <a href="#C-EVID-003" class="claim-link" title="Open source and review card"><code>[C-EVID-003]</code></a>
- [Calibration paper](https://arxiv.org/abs/1706.04599) — calibration on stated distributions. <a href="#C-EVID-004" class="claim-link" title="Open source and review card"><code>[C-EVID-004]</code></a>
- [NASA runtime assurance framework](https://ntrs.nasa.gov/citations/20240010429) — conditional monitor/fallback assurance. <a href="#C-EVID-005" class="claim-link" title="Open source and review card"><code>[C-EVID-005]</code></a>
- [Saliency-map sanity checks](https://arxiv.org/abs/1810.03292) — explanation validation challenge. <a href="#C-EVID-006" class="claim-link" title="Open source and review card"><code>[C-EVID-006]</code></a>
- [Neuron coverage study](https://doi.org/10.1109/MC.2021.3079921) — test-adequacy limitation. <a href="#C-EVID-007" class="claim-link" title="Open source and review card"><code>[C-EVID-007]</code></a>
- [NHTSA automated-driving-systems guidance](https://www.nhtsa.gov/vehicle-manufacturers/automated-driving-systems) — voluntary guidance, not federal approval. <a href="#C-AUTH-008" class="claim-link" title="Open source and review card"><code>[C-AUTH-008]</code></a>
...
<div id="source-cards" class="section">

## Source cards and review limits
...
Model-review refresh is required whenever a manifested claim use changes. Some public-page and scope-only source records retain limited captured context. The live compiler report labels those packages; its completion percentage is review-gate status only, never a safety or certification score.

</div>

<a href="#C-002" class="claim-link" title="Open source and review card">[C-002]</a> U.S. TSO authorization is distinct from installation/use approval

<div class="source-meta">

Project-captured FAA authorization material, represented in the source registry as a limited U.S. approval-scope claim.

Used for: the narrow distinction between equipment authorization and installation or operational use approval.

Limit: local context is short-excerpt only and the current source-support package remains limited. It does not establish an approval chain for a particular DAA product.

</div>

<a href="#C-TRAIN-001" class="claim-link" title="Open source and review card">[C-TRAIN-001]</a> AMLAS data requirements for the vision task

<div class="source-meta">

[AMLAS v1.1](https://www.york.ac.uk/media/assuring-autonomy/documents/AMLASv1.1.pdf), University of York, PDF pp. 21–23 (printed pp. 20–22).

Used for: data relevance, completeness, accuracy, balance, installed sensor viewpoint, lighting and partial-occlusion labels.

Limit: these are component-method requirements and examples, not proof that the hypothetical road or air data meet them.

</div>

<a href="#C-TRAIN-002" class="claim-link" title="Open source and review card">[C-TRAIN-002]</a> AMLAS model learning, overfitting and leakage

<div class="source-meta">

[AMLAS v1.1](https://www.york.ac.uk/media/assuring-autonomy/documents/AMLASv1.1.pdf), PDF pp. 33–35 (printed pp. 32–34).

Used for: overfitting, simulator-artifact example, development log, candidate choices and internal-test leakage.

Limit: the examples identify failure modes; they do not diagnose any real model or show that a specific regularization or split prevents them.

</div>

<a href="#C-TRAIN-003" class="claim-link" title="Open source and review card">[C-TRAIN-003]</a> AMLAS independent challenge and verification

<div class="source-meta">

[AMLAS v1.1](https://www.york.ac.uk/media/assuring-autonomy/documents/AMLASv1.1.pdf), PDF pp. 26, 40–41 (printed pp. 25, 39–40).

Used for: independent verification data, realistic failure classes, unseen inputs and adverse conditions.

Limit: finite verification sets do not prove universal generalization or a vehicle-level safety result.

</div>

<a href="#C-TRAIN-004" class="claim-link" title="Open source and review card">[C-TRAIN-004]</a> Underfitting and overfitting: textbook distinction

<div class="source-meta">

[Deep Learning, Chapter 5, Section 5.2 and Figure 5.3](https://www.deeplearningbook.org/contents/ml.html), Goodfellow, Bengio and Courville.

Used for: the typical relation between model capacity, training error and generalization error.

Limit: a teaching diagram, not a diagnostic proof for a specific vision model or a safety acceptance threshold.

</div>

<a href="#C-CHAL-006" class="claim-link" title="Open source and review card">[C-CHAL-006]</a> AMLAS — learned-component lifecycle

<div class="source-meta">

[Guidance on the Assurance of Machine Learning in Autonomous Systems, v1.1](https://www.york.ac.uk/media/assuring-autonomy/documents/AMLASv1.1.pdf), University of York.

Used for: six lifecycle stages, offline-supervised focus, and the need for complementary system/domain assurance.

Limit: AMLAS is a component methodology, not a vehicle/aircraft approval or whole operational safety case.

</div>

<a href="#C-CHAL-001" class="claim-link" title="Open source and review card">[C-CHAL-001]</a> UK MAA — case-specific military path

<div class="source-meta">

[MAA/RN/2025/04](https://assets.publishing.service.gov.uk/media/68f0b97c1c9076042263ef2a/MAA_RN_2025_04.pdf), UK Military Aviation Authority.

Used for: MCRI agreement, a defined operating context, and fixed supervised models in early applications.

Limit: UK military guidance, not FAA/EASA civil approval or a public product safety case.

</div>

<a href="#C-CHAL-002" class="claim-link" title="Open source and review card">[C-CHAL-002]</a> MAA — architectural mitigation limit

<div class="source-meta">

[MAA/RN/2025/04](https://assets.publishing.service.gov.uk/media/68f0b97c1c9076042263ef2a/MAA_RN_2025_04.pdf), UK Military Aviation Authority.

Used for: case-specific mitigation where a prescriptive path is unavailable.

Limit: full local context is captured, but this is a bounded comparison rather than a full statement of all MAA requirements.

</div>

<a href="#C-CHAL-003" class="claim-link" title="Open source and review card">[C-CHAL-003]</a> EASA DS.AI — proposed civil framework

<div class="source-meta">

[NPA 2025-07(B)](https://www.easa.europa.eu/en/downloads/142701/en), European Union Aviation Safety Agency.

Used for: proposed objectives spanning operating domain, risk, development/learning assurance, lifecycle data, and monitoring.

Limit: an NPA is proposed material, not final AMC/GM or an approval decision.

</div>

<a href="#C-CHAL-004" class="claim-link" title="Open source and review card">[C-CHAL-004]</a> EASA DS.AI — high-consequence boundary

<div class="source-meta">

[NPA 2025-07(B)](https://www.easa.europa.eu/en/downloads/142701/en), European Union Aviation Safety Agency.

Used for: stated exclusion of systems directly contributing to fatalities or multiple life-threatening injuries.

Limit: prevents treating the proposal as an acceptance path for the paper’s hardest flight case.

</div>

<a href="#C-AUTH-002" class="claim-link" title="Open source and review card">[C-AUTH-002]</a> FAA roadmap — frozen release and renewal

<div class="source-meta">

[Roadmap for Artificial Intelligence Safety Assurance](https://www.faa.gov/aircraft/air_cert/step/roadmap_for_AI_safety_assurance), FAA.

Used for: distinction between static learned AI and systems learning in operation; updates receive renewed assurance.

Limit: direction-setting, not a complete means of compliance. Current source-support review is uncertain because the project record has only a short excerpt.

</div>

<a href="#C-AUTH-010" class="claim-link" title="Open source and review card">[C-AUTH-010]</a> FAA roadmap — the learned-implementation traceability gap

<div class="source-meta">

[Roadmap for Artificial Intelligence Safety Assurance, Version I](https://www.faa.gov/aircraft/air_cert/step/roadmap_for_AI_safety_assurance), FAA, PDF p. 14.

Used for: the FAA’s description of why lower-level requirements cannot directly describe a learned algorithm or show ordinary coverage back to higher-level requirements.

Limit: this is FAA’s framing for learned AI in aviation. It does not say that all requirements, item software/hardware traceability, or assurance methods disappear. Full local context is captured for this page; review remains pending after this manuscript revision.

</div>

<a href="#C-AUTH-006" class="claim-link" title="Open source and review card">[C-AUTH-006]</a> ISO/PAS 8800 — road trained-AI scope

<div class="source-meta">

[ISO/PAS 8800:2024](https://www.iso.org/standard/83303.html), ISO.

Used for: public evidence that road safety-assurance work explicitly includes trained AI models.

Limit: scope page only; no clause-level, aviation, or product-conformity claim.

</div>

<a href="#C-AUTH-007" class="claim-link" title="Open source and review card">[C-AUTH-007]</a> UL 4600 — goal-based autonomous-product framework

<div class="source-meta">

[ANSI/UL 4600, Edition 3](https://www.shopulstandards.com/ProductDetail.aspx?productId=UL4600_3_S_20230317), UL Standards & Engagement.

Used for: a public example of a whole-item safety-argument framework.

Limit: active standard, not a government authorization or particular product evaluation.

</div>

<a href="#C-AUTH-008" class="claim-link" title="Open source and review card">[C-AUTH-008]</a> NHTSA ADS guidance — voluntary, not approval

<div class="source-meta">

[Automated Driving Systems](https://www.nhtsa.gov/vehicle-manufacturers/automated-driving-systems), NHTSA.

Used for: distinction between voluntary safety self-assessment and federal approval.

Limit: status needs refresh before publication. Current source-support review is uncertain because full local source context has not been captured.

</div>

<a href="#C-EVID-001" class="claim-link" title="Open source and review card">[C-EVID-001]</a> NIST AI RMF — lifecycle record and limits

<div class="source-meta">

[AI Risk Management Framework 1.0](https://doi.org/10.6028/NIST.AI.100-1), NIST.

Used for: intended use, knowledge limits, evaluation, recovery, and change-process evidence.

Limit: voluntary, not transport-specific, and no acceptance threshold.

</div>

<a href="#C-EVID-002" class="claim-link" title="Open source and review card">[C-EVID-002]</a> Combinatorial methods — selected-factor coverage

<div class="source-meta">

[Assured Autonomy through Combinatorial Methods](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=957175), NIST/Virginia Tech.

Used for: measuring selected scenario-factor combinations and exposing chosen gaps.

Limit: factors and interaction model are analyst choices; omitted causal factors remain omitted.

</div>

<a href="#C-EVID-003" class="claim-link" title="Open source and review card">[C-EVID-003]</a> Rare-event simulation — conditional risk estimates

<div class="source-meta">

[Scalable End-to-End Autonomous Vehicle Testing via Rare-event Simulation](https://arxiv.org/abs/1811.00145), O’Kelly et al.

Used for: conditional risk estimates in a specified simulator and distribution.

Limit: simulator, distribution-tail and hazard-threshold assumptions travel with the result.

</div>

<a href="#C-EVID-004" class="claim-link" title="Open source and review card">[C-EVID-004]</a> Calibration — stated-distribution evidence

<div class="source-meta">

[On Calibration of Modern Neural Networks](https://arxiv.org/abs/1706.04599), Guo et al.

Used for: calibration can be measured/improved on a stated evaluation distribution.

Limit: calibration can fail under operational distribution shift.

</div>

<a href="#C-EVID-005" class="claim-link" title="Open source and review card">[C-EVID-005]</a> Runtime assurance — conditional monitor/fallback proof

<div class="source-meta">

[A Verification Framework for Runtime Assurance of Autonomous UAS](https://ntrs.nasa.gov/citations/20240010429), NASA Langley.

Used for: formal safety implications conditional on monitor observation, time, dynamics, switching, and recovery premises.

Limit: a shared missed perception path can prevent the monitor from observing the failure in time.

</div>

<a href="#C-EVID-006" class="claim-link" title="Open source and review card">[C-EVID-006]</a> Saliency sanity checks — explanation challenge

<div class="source-meta">

[Sanity Checks for Saliency Maps](https://arxiv.org/abs/1810.03292), Adebayo et al.

Used for: testing whether a visual explanation depends on model/data structure.

Limit: interpretation remains a method-specific diagnostic, not a general safety proof.

</div>

<a href="#C-EVID-007" class="claim-link" title="Open source and review card">[C-EVID-007]</a> Neuron coverage — test-adequacy limitation

<div class="source-meta">

[From Neuron Coverage to Steering Angle](https://doi.org/10.1109/MC.2021.3079921), Toohey, Raunak and Binkley.

Used for: empirical motivation to challenge coverage metrics rather than treating them as a guarantee.

Limit: high neuron coverage can coexist with hazardous behavior.

</div>

\[C-RISK-001\] FAA system safety — severity and likelihood context

<div class="source-meta">

[AC 25.1309-1A: System Design and Analysis](https://www.faa.gov/documentLibrary/media/Advisory_Circular/AC_25.1309-1A.pdf), FAA.

Used for: conventional system-safety context in which severity and probability have an inverse relationship.

Limit: historical transport-airplane guidance, not a learned-model acceptance method or a way to turn held-out model performance into a component failure rate.

</div>

\[C-RISK-004\] UK CAA DAA metrics — aggregate-risk limitation

<div class="source-meta">

[CAP3127: Detect and Avoid Policy Concept – Consultation Response Summary](https://www.caa.co.uk/publication/download/25286), UK Civil Aviation Authority.

Used for: risk ratios are relative to their definitions and encounter sets; averages can hide a deficient encounter class.

Limit: consultation/test-phase DAA material, not a universal learned-system criterion or an approved DAA design.

</div>

\[C-RISK-005\] AMLAS Figure 11 — threshold and error-cost teaching pattern

<div class="source-meta">

[Guidance on the Assurance of Machine Learning in Autonomous Systems, v1.1](https://www.york.ac.uk/media/assuring-autonomy/documents/AMLASv1.1.pdf), University of York.

Used for: ROC curves, confusion matrices, selected operating thresholds and contextual error costs.

Limit: its example concerns clinical referral. It is a teaching pattern, not autonomous-road/air performance evidence or a demonstration that beating a human benchmark establishes safety.

</div>

\[C-RISK-006\] EASA proposed DS.AI — authority levels and reserved Level 3B

<div class="source-meta">

[NPA 2025-07(B)](https://www.easa.europa.eu/en/downloads/142701/en), EASA, Table 1.

Used for: Levels 1A–3A's stated end-user authority/responsibility and Level 3B's explicit reserved status.

Limit: proposed aviation material, not a final EASA taxonomy, an SAE mapping, or a general approved “no human in the loop” category.

</div>

\[C-RISK-007\] EASA proposed DS.AI — risk cells, likelihood bands and assurance allocation

<div class="source-meta">

[NPA 2025-07(B)](https://www.easa.europa.eu/en/downloads/142701/en), proposed DS.AI.130, Tables 2–3.

Used for: operational-hour likelihood bands, H1's unacceptable cells, and the proposed route from non-unacceptable aggregated scenarios to AL/TQL.

Limit: not a loss-of-life-per-mile metric, global industry threshold, final EASA rule, or a substitute for a system safety case.

</div>

\[C-DIR-001\] NCEES PE taxonomy — a narrow credential observation

<div class="source-meta">

[PE Exam](https://ncees.org/exams/pe-exam/), National Council of Examiners for Engineering and Surveying.

Used for: the current NCEES list of discipline-specific PE examinations, including computer engineering and no machine-learning specialty.

Limit: it is not a global credential census or a causal explanation of standards maturity, competence, or legal authorization.

</div>

\[C-DIR-002\] NHTSA EV/hybrid comparison — denominator limitation

<div class="source-meta">

[2016 NHTSA quiet-car final rule](https://www.govinfo.gov/content/pkg/FR-2016-12-14/pdf/2016-28804.pdf), Federal Register p. 6.

Used for: the documented absence of powertrain-specific VMT for NHTSA's then-current pedestrian/pedalcyclist comparison.

Limit: an older low-speed audibility rulemaking, not a current all-crash EV rate or an ADS study.

</div>

\[C-DIR-003\] NHTSA Standing General Order — incident data, not a rate

<div class="source-meta">

[SGO 2021-01 Data Dictionary](https://static.nhtsa.gov/odi/ffdd/sgo-2021-01/SGO-2021-01_Data_Element_Definitions.pdf), NHTSA, September 2026.

Used for: current public-data limitations including unverified/incomplete reports, duplicate reports and non-normalized summaries.

Limit: incident discovery and oversight data; it does not establish comparative exposure, a manufacturer finding, or learned-system safety.

</div>

\[C-STD-001\] FAA AI/ML discipline — active collaboration

<div class="source-meta">

[Technical Discipline: Artificial Intelligence – Machine Learning](https://www.faa.gov/aircraft/air_cert/step/disciplines/artificial_intelligence), FAA.

Used for: FAA's stated collaboration with industry, government, standards-development organizations and academia in the aviation certification context.

Limit: a program-activity page, not a standard, means of compliance, or product approval.

</div>

\[C-STD-002\] EASA DS.AI — proposed rulemaking status

<div class="source-meta">

[RMT.0742 / NPA 2025-07 status page](https://hub.easa.europa.eu/crt/docs/by_npa_ref/dir_1/dpp_50), EASA.

Used for: the captured status label that DS.AI material is proposed and awaiting comment responses.

Limit: time-sensitive website status. Recheck final decisions, CRDs and AMC/GM before publication.

</div>

\[C-STD-003\] U.S. FMVSS compliance certification statute

<div class="source-meta">

[49 U.S.C. § 30115](https://uscode.house.gov/view.xhtml?edition=prelim&num=0&req=granuleid%3AUSC-prelim-title49-section30115), Office of the Law Revision Counsel.

Used for: manufacturer/distributor certification of compliance with applicable FMVSS and the reasonable-care limitation.

Limit: this does not prescribe an ML development method, authorize an operation, or prove a model handles every scenario safely.

</div>

\[C-STD-004\] NHTSA — no federal pre-approval of ADS technology

<div class="source-meta">

[2025 NHTSA interpretation](https://www.nhtsa.gov/interpretations/ncc-230607-001-571108-automatic-activation-hazard-warning-signal-nonresponsive).

Used for: NHTSA's stated distinction between pre-approval, applicable-FMVSS self-certification and defect authority.

Limit: nonbinding interpretation of a particular ADAS/lighting question. The project has browser-inspected it but still needs a full retained local snapshot before release-grade reliance.

</div>

</div>

<div id="current-compiler-status" class="section live-status" aria-label="Current verification status">
...
- **Worked-case gate completion:** 72.8% (67 passing gates of 92).
- **Automated pre-human review:** 97.1%.
- **Human disposition:** 0.0%.
- **Open case blockers:** 40.
