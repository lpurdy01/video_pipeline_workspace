...
Bounded LLM Review for Continuous Verification: A Graph-Decomposition Architecture for Safety-Critical Software
...
DAG Node  (code unit + requirement chain + tool evidence)
       ...
       └─► Verification Query Package  (deterministic assembly)
                      │
                      ├─► Human engineer   (certification pass — qualified, independent reviewer)
                      └─► LLM model        (nightly CI — development feedback only)

Specialized tool outputs (static analysis, coverage, timing) enter as DAG evidence nodes,
not as reviewer endpoints — see Figure 4.
```
...
REQUIREMENTS
  ...
  LL-1: read_sensor() shall mask the raw ADC output to 12 bits before returning.
...
STATIC ANALYSIS (SA-001): No integer overflow, no uninitialized reads detected.
...
```json
{
  ...
  "evidence_citations": ["ADC_Datasheet_RevB §3.2", "TC-001", "TR-001", "SA-001"],
  "flags": []
}
```
